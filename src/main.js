...
import { setSky } from "./modules/sky.helper.js";
const sky_path = "./SKY.hdr";
// Scene setup
scene.add(ground);
scene.add(cloudLayer);
loadCities(scene);
setSky(scene, sky_path);
...
} = setupGUI(parachutist, airplane);

// Load models and then initialize controls and animation
Promise.all([airplane.load(scene), parachutist.load(scene)]).then(() => {
  // Controls
  ...
  function animate() {
    ...
    if (parachutist.hasJumped) {
      ...
      console.log(
        "updated the camera and the position of the parachutist is :",
        parachutist.position,
      );
    ...
    const target =
      !parachutist.hasJumped && airplane.mesh
